...
8. **Fallback when `webSearch` is unavailable on Vercel** — when a Vercel-backend user hasn't set `TAVILY_API_KEY`, `webSearch` is a no-op. Realistically most users will have search via a native backend (`claude`/`codex`/`openai`) or set up Tavily, but the no-search case is real. A future `web-research` skill could describe strategies the model can use without a search tool: scrape DuckDuckGo HTML via `browser_fetch` (more scrape-friendly than Google, no JS required, raw SERPs), navigate from a known landing page (Wikipedia, official docs), or ask the user. Skill belongs in `spec/skills/web-research.md` when the case shows up; v1 ships without it because a missing search tool is a configuration choice the user made deliberately.
