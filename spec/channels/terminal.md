...
- **On Enter (before sending):** expand any placeholder tokens in the line to their stashed content, then send. readline's normal behavior (show the typed line, move cursor to next row, emit `line`) handles the display; no manual clear of wrapped rows is needed because the in-buffer content is always short.
