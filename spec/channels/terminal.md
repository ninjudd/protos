...
- **Multiline paste works as one message.** Enable **bracketed paste mode** at client startup (`\x1b[?2004h`) and restore on exit (`\x1b[?2004l`). Terminals that support it wrap pasted content in `\x1b[200~ … \x1b[201~` markers. The client treats everything between those markers as a single input chunk — newlines inside a paste are preserved as literal `\n` in `input`, NOT as Enter/submit events. The user can still edit the pasted content before pressing Enter; Enter then submits the whole multi-line message as one `{type: "message", text: "..."}` (with embedded newlines).

  **Newline normalization in raw mode.** stdin is in raw mode (`ICRNL` disabled), so pasted newlines arrive as bare `\r`, not `\n`. Normalize pasted content: replace every `\r\n` and bare `\r` with `\n` before storing in `input` and before echoing. When echoing to the terminal, translate each `\n` back to `\r\n` (raw mode also disables `ONLCR`, so `\n` alone won't advance to column 0 of the next row and the rows overwrite each other).
- **On Enter (before sending):** the user's typed line is visible on the prompt line (`❯ hello?`), possibly wrapped across multiple terminal rows for long or multi-line input. The client then **clears every row the input occupied** (compute from prompt width + input length + embedded newlines + terminal width, then emit one cursor-up + clear-line per row) so the line can be redrawn uniformly by the render path when the server echoes it back. Clearing only the final row leaves the wrapped prefixes on screen and the echoed message appears *in addition to* the leftover fragments — giving a cascade of `❯ ...` on long input. Otherwise the same message appears twice — once as raw typed input, once as the rendered echo.
