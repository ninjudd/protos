...
- **On Enter:** expand any placeholder tokens in the line to their stashed content, send to server as `{type: "message", text: "..."}`, and **clear the typed line from the screen** (the row(s) readline echoed as you typed). The displayed message comes from the server's broadcast, not the local readline echo — so the canonical, server-acknowledged version is what the user sees, and any future server-side processing (normalization, transformation) is reflected accurately.
