...
- **Enter key behavior:**
  - **Plain Enter with non-empty input:** expand any placeholder tokens in the line to their stashed content, send to server as `{type: "message", text: "..."}`, and **clear the typed row(s) from the screen** (account for content wrapped across multiple terminal rows). The displayed message comes from the server's broadcast, not the local readline echo — so the canonical, server-acknowledged version is what the user sees, and any future server-side processing (normalization, transformation) is reflected accurately.
  - **Plain Enter with empty or whitespace-only input:** no-op. Don't send, don't clear the prompt.
  - **Shift+Enter or Alt+Enter:** insert a literal `\n` into the input buffer at the cursor position without submitting. The client intercepts the raw byte sequences before readline's submit path: Alt+Enter arrives as `ESC\r` (`0x1B 0x0D`); Shift+Enter arrives as the kitty-keyboard-protocol sequence `ESC[13;2u` (terminals like iTerm2 can be configured to emit this). Bind both so users on either terminal style get multi-line input.
