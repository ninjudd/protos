...
Pipeline: `chromium.launch({ headless: true })` → fresh `BrowserContext` (cookie isolation) → `page.goto(url, { waitUntil: 'networkidle', timeout: 30000 })` (catch the timeout — see "Networkidle is a best-effort wait" below) → `page.content()` → Mozilla Readability over the rendered HTML → return `{ url: page.url(), title: article.title, content: article.textContent }`.

**Networkidle is a best-effort wait, not a must-succeed gate.** Wrap `page.goto` in a try/catch and proceed to `page.content()` even when the timeout fires. Continuously-polling SPAs (X/Twitter, news live-blogs, dashboards), analytics beacons, and ad telemetry keep the network busy past 30s on many sites — `networkidle` never fires, the goto throws, but the page is usually fully rendered and readable by that point. The reference probe at `~/bin/playwright-probe` does exactly this; the production tool must too. Heuristic for distinguishing "timed out but rendered" from "actually failed": `await page.content()` after the timeout, and if the result is non-trivial (e.g. >200 chars and contains a `<body>`), treat the fetch as successful. If `page.content()` is empty or the page literally never loaded (DNS error, connection refused), *then* surface a `browser_fetch: navigation timeout for {url}` error.
...
- **Failure modes** surface as tool errors (the SDK's tool-dispatch layer turns thrown errors into tool results the model sees). **Wrap-all rule:** every error thrown from this tool prefixes its message with `browser_fetch:` and includes the URL where one is in scope — including failures from `chromium.launch`, `launchPersistentContext`, `newContext`, `newPage`, `safeSegment` on the session name, and the navigation/render path below. The model gets a consistent surface and can reliably detect "this is a browser_fetch failure, fall back to webFetch." Common cases (the message shapes for the most frequent paths):
  - Navigation timeout (>30s) **without rendered content** → `Error("browser_fetch: navigation timeout for {url}")`. A timeout *with* rendered content is **not** an error — return the rendered text per the "Networkidle is a best-effort wait" rule above.
